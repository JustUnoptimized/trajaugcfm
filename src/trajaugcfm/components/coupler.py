...
class RBFCoupler(Coupler):
    ...
    def sample(
        ...
        refs_snaps = refs[:, Tsnaps, :]

        # Compute pairwise squared euclidean distances
        # || x - y ||^2 = || x ||^2 - 2<x, y> + || y ||^2
        Xos2 = (Xos ** 2).sum(dim=2, keepdim=True).transpose(1, 2)  # (N, 1, snaps)
        refs2 = (refs_snaps ** 2).sum(dim=2, keepdim=True).transpose(1, 2)  # (r, 1, snaps)
        inner = torch.einsum('nsi,rsi->rns', Xos, refs_snaps)  # (r, N, snaps)
        # Ds[i, j, s] contains ||xi - yj||^2 at snapshot s
        Ds = Xos2.transpose(0, 1) - (2 * inner) + refs2  # broadcast sum

        # RBF weight each distance and normalize using logsumexp trick
        logRBFs = Ds / self.denom
        logRBFs -= logRBFs.max(dim=1)
        RBFs = torch.exp(logRBFs)
        normconst = RBFs.sum(dim=1, keepdim=True)  # (r, 1, snaps)
        RBFs /= normconst
        ...
        u = torch.rand(
            (refs.shape[0], k, snaps),
            generator=self.gen,
            device=self.device
        )  # (r, k, snaps)
        z_idxs = torch.searchsorted(
            RBFs_CDF.transpose(1, 2).contiguous(),  # (r, snaps, N)
            u.transpose(1, 2).contiguous(),  # (r, snaps, k)
        ).transpose(1, 2)  # (r, k, snaps)
        ...
        assert z.shape == (refs.shape[0], k, snaps, Xs.shape[-1])
